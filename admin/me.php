...
if (isset($_POST['add'])) {
	...
	if($_POST['aksi']=='edit')
	{
		...
		$sql = mysqli_query($connect,"UPDATE berita SET Judul='".$_POST['judul']."',Isi='".$_POST['isi']."',Kategori='".$_POST['kategori']."',Teks='".$_POST['teks']."',Terbit='".$_POST['terbit']."' WHERE ID='".$_POST['id']."' ");
	}
...
<div class="w100">
	<form action="./?mod=berita" method="POST" enctype="multipart/form-data">
		...
		<fieldset>
			...
			<input class="w100" type="text" name="judul" placeholder="Judul berita" value="<?=(isset($judul) ? $judul : '')?>" size="40" required>
			</div>
			...
			<textarea name="isi" rows="8" class="summernote" required><?=(isset($isi) ? $isi : '')?></textarea>
			...
			<div class="formnama">
			<label>Teks</label>:<br>
			<textarea class="w100" name="teks" rows="5" required><?=(isset($teks) ? $teks : '')?></textarea>
			</div>
